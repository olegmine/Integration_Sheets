...
async def update_data_ozon():
    ...
    if not price_changed_df.empty:
        await update_prices_ozon(price_changed_df, "t_price", "old_price", "offer_id", "min_price", 'client_id', 'api_key', debug=True)
...
async def update_data_wb():
    ...
    if not price_changed_df.empty:
        await update_prices_wb(price_changed_df, "nmID", "t_price", "discount", 'api_key', debug=True)
...
async def update_data_ym():
    ...
    if not price_changed_df.empty:
        await update_price_ym(price_changed_df, 'access_token', 'campaign_id', "offer_id", "t_price", "discount_base", debug=True)
...
async def update_data_mm():
    ...
    if not price_changed_df.empty:
        await update_prices_mm(price_changed_df, 'token', "offerId", "t_price", "isDeleted", debug=True)
